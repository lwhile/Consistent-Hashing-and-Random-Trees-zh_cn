...
Malpani等人想要把一组缓存函数封装成一个来解决这个问题（#TODO）。用户的一个页面请求直接去到任意一份缓存。如果页面存在，就直接返回给用户。否则，缓存通过一个叫“IP Multicast”的特殊协议发送其他所有的缓存。如果所有缓存都没有这个页面，请求会发送到该页面所在的主站。这个方案的坏处就在于，参与的缓存越来越多，即使用组播，缓存之间的信息也会变得越来越难维护。我们将在第四节讨论这篇论文所介绍的一致性哈希，它提供了一种方法去实现这种无需通信的分布式缓存。
...
在这一节，将展示我们在处理网络热点问题的模型。

我们将在互联网上的计算机分成三类。全部web页面请求由*浏览器*发起。*服务器*永久保存了全部Web页面。*缓存*则是额外的用来保护服务器免受大量的浏览器请求。在这篇论文里，缓存能存放的页面数量为*c*以及缓存的个数为*C*。

每一个服务器都保存着固定数量的页面。缓存同样页保存着一些页面，但是缓存里页面的集合会受到一个缓存协议的影响，并随着时间而改变。通常，我们假设每个页面的内容是不变的，但我们会在第9章节讨论这个问题。我们定义所有页面的集合为Р。

任何一个机器都能在以下限制下向别的机器直接发送消息，这个限制是指一个机器可能并不能意识到全部缓存机器的存在。我们只要求每台机器只意识到1/t个缓存，t是个常量。有两种典型的消息类型，页面请求以及页面本身的请求。一台机器在短时间收到大量消息而导致无法正常运行，这种情况叫做“过载”。

*延迟*是指从机器m1发出消息到m2接收到这条消息所需要的时间。我们将这个数值定义为 δ(m1，m2)。当然，在实践中，网络延迟并不能简单地测量出来（#TODO）。
这个δ值应当作为一个“最佳估值”，因为我们是在缺乏更优的信息的基础上优化它的（#TODO）。一个协议的正确性不应该依赖f值的精确（还可以从吞吐量，连接的代价以及拥挤程度考量）。注意，我们还没考虑消息大小对延迟的影响。这个问题在章节3.2.1里讨论。

所有的缓存和服务器以及浏览器的行为在我们的协议里都有规定。尤其是这个协议规定了缓存和服务器是如何回应页面请求以及哪些页面要存储在一个缓存里面。这个协议页规定了一个浏览器请求应该发给缓存还是服务器。所有的控制必须在本地；一台机器的行为只能依赖于它收到的消息。

一个使用浏览器的人决定了要请求哪些页面。然而，他看不到在我们协议里面生成的随机值，在获取页面的时候也无法根据延迟来改变他的请求（#TODO）。我们考虑两种模型。第一，我们考虑一个不可变的模型，在单一“批次”的请求同时需要处理的情况，要求页面请求的数量最多是R=ρC，ρ是一个常量以及C是缓存的个数。然后我们考虑一个暂时的模型，一个人发起新的页面请求的频率最大为γ；在任何时间间隔τ > 1的情况下，他最多发起γτ个请求。

### 目的

”热点问题“是确保在很大概率上没有缓存或服务器过载的前提下，能够满足全部浏览器的页面请求。“很大概率上”指的是“至少1-1/N的概率“，在这篇论文里，N是个确切的参数（#TODO）。

我们的基本要求是避免超负荷，同时我们还有两个其他的目标。第一个目标就是减少缓存的内存需要。一个协议不应该要求任何缓存存储很大数量的页面才能良好运行。第二个目标自然是减少一个浏览器获取一个页面的过程的延迟。
...
在接下来的3.1，我们会详尽地定义我们的协议。在接下来3.2，我们会通过限制任意缓存的负载、每个缓存使用的容量和浏览器获取页面的延迟这几个方面来分析我们的协议。

##3.1 协议

我们将一个称为抽象树的有根d-ary树与每个页面相关联。我们仅使用术语“节点”来引用这些抽象树的节点。每棵树中的节点数等于缓存的数量，并且树尽可能平衡（因此所有级别除了底部的都已满了）。我们通过使用广度优先搜索得到的排名来引用树的节点。该协议需要在这些抽象树上运行;为了支持这一点，所有页面请求都采用4元组的形式，包括请求者的身份，所需页面的名称，重定向的节点序列，以及应该充当那些节点的一系列缓存（#TODO）。为了确定较近的一个序列（#TODO），即哪个缓存实际上为给定节点工作，节点被映射到机器（#TODO）。 树的根始终映射到服务器页面。其余的全部节点都通过用可以分布到所有的浏览器和缓存的哈希函数（h：公式稍后带上）被映射到缓存（#TODO）。为了避免为那些请求很少的页面也创建副本，我们有另一个参数 q ，用于在缓存存下了页面的副本前，必须让它先了解这个页面的请求数。
