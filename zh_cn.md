...
我们将在互联网上的计算机分成三类。全部web页面请求由*浏览器*发起。*服务器*永久保存了全部Web页面。*缓存*则是额外的用来保护服务器免受大量的浏览器请求。在这篇论文里，缓存能存放的页面数量为*С*以及缓存的个数为*C*。

每一个服务器都保存着固定数量的页面。缓存同样页保存着一些页面，但是缓存里页面的集合会受到一个缓存协议的影响，并随着时间而改变。通常，我们假设每个页面的内容是不变的，但我们会在第9章节讨论这个问题。我们定义所有页面的集合为Р。

任何一个机器都能在以下限制下向别的机器直接发送消息，这个限制是指一个机器可能并不能意识到全部缓存机器的存在。我们只要求每台机器之意识到1/t个缓存，t是个常量。有两种典型的消息类型，页面请求以及页面本身的请求（#TODO）。一台机器在短时间收到大量消息而导致无法正常运行，这种情况叫做“超负荷”。
...
”热点问题“是确保在很大概率上没有缓存或服务器是swamped的的前提下，能够满足全部浏览器的页面请求。“很大概率上”指的是“至少1-1/N的概率“，在这篇论文里，N是个确切的参数（#TODO）。
